...
for (const mode of ['widget', 'fallback']) test(`card checkout ${mode} preserves term and collects no card data`, async t => {
  const context = await browser.newContext({ viewport: { width: mode === 'widget' ? 1440 : 390, height: 1000 } });
  ...
  await context.route('**/*', route => {
    ...
    if (url.hostname === 'pay.hotmart.com') { targetUrl = url; return route.fulfill({ contentType: 'text/html', body: 'Checkout de teste interceptado' }); }
  ...
  if (mode === 'widget') await page.waitForFunction(() => Boolean(window.jQuery?.fancybox));
  ...
  assert.equal(posts, 0, 'no buyer or card data posted to legacy checkout');
